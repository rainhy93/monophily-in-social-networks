...
def LINK(num_unlabeled, membership_y, feature_x, clf, cv_setup=None):
    ...
    for i in range(len(num_unlabeled)):
        ...
        for (train, test) in k_fold.split(feature_x, membership_y):
            ...
            if len(np.unique(membership_y))>2:
                ...
                wt_auc.append(metrics.roc_auc_score(label_binarize(membership_y[test],np.unique(membership_y)), prob, average = 'weighted'))
            else:
